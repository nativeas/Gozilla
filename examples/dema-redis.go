...
func main() {
	...
	/*[Test HMSET 标准参数转换数据]*/
	player, e := redisobj.HMSet(localdb.TransHMSetDB("fengjie", localdb.PlayerobjField, p)[0:]...)
	if e != nil {
		log.Println(e)
	}
	log.Println(player)
	/*[Test HMGET 获取单个键值数据]*/
	playerdata, _ := redisobj.HMGet(localdb.TransHMGetDB("fengjie", localdb.PlayerobjField)[0:]...)
